    ...
    private void configureWebView() {
        ...
        web.setWebViewClient(new WebViewClient() {
            ...
            public void onPageFinished(WebView view, String url) {
                ...
                view.evaluateJavascript(
                        "(function(){try{localStorage.removeItem('mahindra_secure_token_v1');localStorage.removeItem('mahindra_secure_user_v1');localStorage.removeItem('mahindra_user_role_v2');}catch(e){};var s=document.createElement('script');s.src='file:///android_asset/app_shell.js';document.head.appendChild(s);setTimeout(function(){if(window.__mahindraLoginWatchdog)return;window.__mahindraLoginWatchdog=true;var old=window.appHttpResult;function loginScreen(){if(document.getElementById('mahindraNativeLogin'))return;var x=document.createElement('div');x.id='mahindraNativeLogin';x.style.cssText='position:fixed;inset:0;z-index:2147483647;background:#f5f7fa;display:flex;align-items:center;justify-content:center;padding:24px;box-sizing:border-box;font-family:Arial,sans-serif';x.innerHTML='<div style=\"width:min(420px,100%);background:#fff;border-radius:22px;padding:26px;box-shadow:0 8px 30px #0002\"><div style=\"font-size:46px;text-align:center\">🚛</div><h1 style=\"margin:8px 0;text-align:center;color:#b71c1c;font-size:26px\">MAHINDRA BSVI</h1><p style=\"text-align:center;color:#667085;margin:0 0 22px\">Driver & Mechanic Guide AI • Secure Login</p><label style=\"font-weight:800\">10-Digit Mobile Number</label><input id=\"mlMobile\" inputmode=\"numeric\" maxlength=\"10\" style=\"width:100%;box-sizing:border-box;padding:14px;margin:7px 0 14px;border:1px solid #d0d5dd;border-radius:10px;font-size:17px\"><label style=\"font-weight:800\">Password</label><input id=\"mlPass\" type=\"password\" maxlength=\"32\" style=\"width:100%;box-sizing:border-box;padding:14px;margin:7px 0 14px;border:1px solid #d0d5dd;border-radius:10px;font-size:17px\"><button id=\"mlBtn\" style=\"width:100%;padding:15px;border:0;border-radius:10px;background:#b71c1c;color:#fff;font-size:17px;font-weight:900\">🔐 LOGIN</button><div id=\"mlErr\" style=\"display:none;color:#b71c1c;font-weight:700;margin-top:12px\"></div><div style=\"color:#667085;font-size:12px;margin-top:14px\">Driver/Mechanic accounts use the last 4 digits of the mobile as the initial password.</div></div>';document.body.appendChild(x);x.querySelector('#mlBtn').onclick=function(){var m=x.querySelector('#mlMobile').value.replace(/\\D/g,'');var p=x.querySelector('#mlPass').value;var e=x.querySelector('#mlErr');if(!/^\\d{10}$/.test(m)||!p){e.style.display='block';e.textContent='Enter the 10-digit mobile number and password.';return;}x.querySelector('#mlBtn').disabled=true;x.querySelector('#mlBtn').textContent='⏳ LOGGING IN...';window.appHttpResult=function(raw){try{var outer=JSON.parse(raw);var result=JSON.parse(outer.body||'{}');if(result.ok){localStorage.setItem('mahindra_secure_token_v1',result.token||'');localStorage.setItem('mahindra_secure_user_v1',JSON.stringify(result.user||{}));localStorage.setItem('mahindra_user_role_v2',String(result.user&&result.user.role||'').toLowerCase());x.remove();window.appHttpResult=old;if(typeof window.routeAfterLogin==='function')window.routeAfterLogin();else if(typeof window.showLogin==='function')window.showLogin();}else{e.style.display='block';e.textContent=result.error||'Login failed.';x.querySelector('#mlBtn').disabled=false;x.querySelector('#mlBtn').textContent='🔐 LOGIN';window.appHttpResult=old;}}catch(err){e.style.display='block';e.textContent='Login response could not be read.';x.querySelector('#mlBtn').disabled=false;x.querySelector('#mlBtn').textContent='🔐 LOGIN';window.appHttpResult=old;}};window.AndroidBridge.cloudRequest('POST',window.AndroidBridge.getSharedApiUrl(),JSON.stringify({action:'login',username:m,password:p}));};}function ensureLogin(){var has=document.getElementById('secureAuth')||document.getElementById('mahindraNativeLogin');if(has)return;if(typeof window.showLogin==='function'){try{window.showLogin();if(document.getElementById('secureAuth'))return;}catch(e){}}loginScreen();}setTimeout(ensureLogin,900);setTimeout(ensureLogin,2500);setTimeout(ensureLogin,5000);},1800);})();",
                        null
                );
            }
