    ...
    private void configureWebView() {
        ...
        web.setWebViewClient(new WebViewClient() {
            ...
            public void onPageFinished(WebView view, String url) {
                ...
                // Keep this parser deliberately dependency-free: direct JSON, meta transport, or HTML text.
                view.evaluateJavascript(
                        "(function(){window.__mahindraNormalizeResponse=function(raw){"
                                ...
                                + "}catch(e){return null;}}"
                                + "function meta(v){"
                                + "var p=v.indexOf('name=\\\"mahindra-json\\\"');"
                                + "if(p<0)p=v.indexOf("name='mahindra-json'");"
                                + "if(p>=0){var q=v.indexOf('content=',p);if(q>=0){var ch=v.charAt(q+8);if(ch===\\\"\\\"||ch===\"'\"){var z=v.indexOf(ch,q+9);if(z>q)return v.slice(q+9,z);}}}"
                                + "p=v.indexOf('content=\\\"');if(p<0)p=v.indexOf("content='");"
                                + "if(p>=0){var ch=v.charAt(p+8);var z=v.indexOf(ch,p+9);if(z>p)return v.slice(p+9,z);}"
                                + "return null;"
                                + "}"
                                + "function extract(v){"
                                + "v=v||'';var direct=parse(v.trim());if(direct)return direct;"
                                + "var mv=meta(v);if(mv){var j=parse(b64(mv)||'');if(j)return j;}"
                                + "var box=document.createElement('div');box.innerHTML=v;var t=(box.textContent||box.innerText||'').trim();"
                                + "j=parse(t);if(j)return j;"
                                + "var a=t.indexOf('{'),z=t.lastIndexOf('}');if(a>=0&&z>a){j=parse(t.slice(a,z+1));if(j)return j;}"
                                + "a=v.indexOf('{');z=v.lastIndexOf('}');if(a>=0&&z>a){j=parse(v.slice(a,z+1));if(j)return j;}"
                                + "return null;"
                                + "}"
                                + "try{"
                                + "var o=(typeof raw==='string')?JSON.parse(raw):raw;var b=o&&o.body;var j=extract(typeof b==='string'?b:'');"
